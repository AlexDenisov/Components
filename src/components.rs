...
fn main() {
    ...
    if explain_or_command == "explain" {
         command = args.next().unwrap_or_else(|| {
             println!("provide a command to explain: install, update, purge etc");
             process::exit(1);
         });
    ...
    for path in component_paths {
        ...
        unsafe {
            ...
            let (stdout, stderr) = if debug_mode || explain_mode { 
                (Stdio::inherit(), Stdio::inherit())
            } else { 
                (Stdio::from_raw_fd(fd), Stdio::from_raw_fd(fd))
            };
